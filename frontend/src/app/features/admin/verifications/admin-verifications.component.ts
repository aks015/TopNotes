...
@Component({
      ...
      <header class="vf-head">
        ...
        <p class="vf-sub">
          @if (loading()) {
            ...
            <b>{{ pending().length }}</b> per-category
            {{ pending().length === 1 ? 'qualification' : 'qualifications' }} awaiting review.
          }
        </p>
      </header>
        ...
        <div class="vf-grid">
          @for (p of pending(); track p.id) {
            <div class="vf-card">
              ...
              <div class="vf-meta">
                <div>
                  <span>Email</span><b>{{ p.email }}</b>
                </div>
                <div>
                  <span>Submitted</span><b>{{ p.submittedAt ? (p.submittedAt | date: 'd MMM y, h:mm a') : '—' }}</b>
                </div>
              </div>

              <div class="vf-ms-label">Marksheet</div>
              @if (p.marksheetUrl) {
                @if (isPdf(p.marksheetUrl)) {
                  <a class="vf-ms vf-ms-pdf" [href]="p.marksheetUrl" target="_blank" rel="noopener">
                    <span class="vf-pdf-ic">📄</span>
                    <span>Open marksheet PDF</span>
                  </a>
                } @else {
              ...
              <label class="vf-check">
                <input
                  type="checkbox"
                  [checked]="nameMatched()[p.id] || false"
                  (change)="setNameMatch(p.id, $any($event.target).checked)"
                />
                <span
                  >Marksheet name matches the account name (<b>{{ p.sellerName }}</b
                  >).</span
                >
              </label>
              ...
              @if (p.institution) {
                <label class="vf-check">
                  <input
                    type="checkbox"
                    [checked]="instMatched()[p.id] || false"
                    (change)="setInstMatch(p.id, $any($event.target).checked)"
                  />
                  <span
                    >Institution matches the marksheet (<b>{{ p.institution }}</b
                    >).</span
                  >
                </label>
              }
              ...
              <div class="vf-actions">
                ...
                <button
                  class="vf-btn approve"
                  [disabled]="busyId() === p.id || !nameMatched()[p.id] || (!!p.institution && !instMatched()[p.id])"
                  (click)="approve(p)"
                >
    ...
    @if (rejecting(); as r) {
      <div class="vf-modal-scrim" (click)="rejecting.set(null)">
        <div class="vf-modal" (click)="$event.stopPropagation()">
          ...
          <p class="vf-modal-sub">
            Rejecting <b>{{ r.sellerName }}</b
            >. Give a reason — they'll see it and can re-submit.
          </p>
          <textarea
            class="vf-textarea"
            ...
            placeholder="e.g. Marksheet image is blurry / doesn't match the category…"
          ></textarea>
          @if (reasonError()) {
            <div class="vf-err">A reason is required.</div>
          }
  ...
  styles: [
    `
      ...
      .vf-btn.approve:hover:not(:disabled) {
        background: #16864f;
      }
      .vf-btn.reject {
        background: #fff;
      ...
      .vf-btn.reject:hover:not(:disabled) {
        background: #fdeceb;
      }
      .vf-btn.ghost {
        background: #f0ede4;
      ...
      .vf-skel {
        ...
        animation: vfShimmer 1.3s infinite;
      }
      @keyframes vfShimmer {
        from {
...
export class AdminVerificationsComponent {
  ...
  /** Per-card verification checklist — both must be ticked (where applicable) to Approve. */
  protected nameMatched = signal<Record<number, boolean>>({});
  protected instMatched = signal<Record<number, boolean>>({});
  ...
  /** Marksheet stored as a PDF (vs an image) → preview as a link, not <img>. */
  protected isPdf(url: string): boolean {
    return /\.pdf($|\?)/i.test(url);
  }

  protected setNameMatch(id: number, checked: boolean) {
    this.nameMatched.update((m) => ({ ...m, [id]: checked }));
  }
  protected setInstMatch(id: number, checked: boolean) {
    this.instMatched.update((m) => ({ ...m, [id]: checked }));
  }

  protected approve(p: QualificationReview) {
    // Guard: name-match always required; institution-match required when one was provided.
    if (!this.nameMatched()[p.id]) return;
    if (p.institution && !this.instMatched()[p.id]) return;
    this.busyId.set(p.id);
    this.api
      .reviewQualification(p.id, true)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: () => {
          this.busyId.set(null);
          this.toast.success(`${p.sellerName} approved for ${p.categoryName}`);
          this.remove(p.id);
        },
        error: () => this.busyId.set(null),
      });
  }
  protected openReject(p: QualificationReview) {
  ...
  protected confirmReject() {
    ...
    this.api
      .reviewQualification(p.id, false, this.reason().trim())
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: () => {
          this.toast.success(`${p.sellerName}'s ${p.categoryName} qualification rejected`);
